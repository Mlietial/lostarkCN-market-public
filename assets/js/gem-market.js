(() => {
  ...
  const auctionRateByDate = (() => {
    const rates = new Map();
    (dashboard?.goldTxns || []).forEach(txn => {
      if (txn?.type !== "拍卖交易" || !txn.date || !finite(txn.gold) || !finite(txn.price) || Number(txn.price) <= 0) return;
      const rate = Number(txn.gold) / Number(txn.price);
      if (!finite(rate)) return;
      rates.set(txn.date, Math.max(rate, rates.get(txn.date) || 0));
    });
    return rates;
  })();
  ...
  const rmb = value => finite(value) ? `¥${fmt(value, Math.abs(Number(value)) >= 100 ? 1 : 2)}` : "—";
  ...
  function gemForUnit(gem) {
    const points = pointsForUnit(gem);
    const current = points.at(-1)?.value;
    const previous = points.at(-2)?.value;
    const weekBase = points.at(-8)?.value ?? points[0]?.value;
    const recent = points.slice(-30).map(point => point.value);
    const returns = points.slice(1).map((point, index) => (point.value - points[index].value) / points[index].value).filter(finite);
    const recentReturns = returns.slice(-30);
    const returnMean = mean(recentReturns);
    const variance = mean(recentReturns.map(value => (value - returnMean) ** 2));
    const high30 = recent.length ? Math.max(...recent) : null;
    const low30 = recent.length ? Math.min(...recent) : null;
    return {
      ...gem,
      current,
      previous,
      ...
      average30: mean(recent),
      volatility: Math.sqrt(variance),
      position: finite(high30) && finite(low30) && high30 !== low30 ? (current - low30) / (high30 - low30) : .5,
      points
    };
  }
  ...
  function tickerMarkup() {
    ...
    const group = clone => `<div class="ticker-group" ${clone ? 'aria-hidden="true"' : ""}>${groupItems.map((gem, index) => {
      ...
      return `<button class="ticker-item" type="button" data-gem="${gem.id}" ${duplicate ? 'tabindex="-1" aria-hidden="true"' : ""}><strong>${view.name}</strong><b>${priceShort(view.current)}</b><span class="${tone(view.dayChange)}">${pct(view.dayChange)}</span></button>`;
    }).join("")}</div>`;
  ...
  function movesMarkup(gem) {
    return biggestMoves(gem).map(move => `<div class="move-row"><time>${shortDate(move.date)}</time><strong>${priceShort(move.value)}${state.unit === "gold" ? " 金" : ""}</strong><b class="${tone(move.change)}">${pct(move.change)}</b></div>`).join("");
  }

  function syncTickerValues() {
    terminal.querySelectorAll(".ticker-item[data-gem]").forEach(item => {
      const gem = gems.find(row => row.id === item.dataset.gem);
      if (!gem) return;
      const view = gemForUnit(gem);
      const value = item.querySelector("b");
      const change = item.querySelector("span");
      if (value) value.textContent = priceShort(view.current);
      if (change) {
        change.className = tone(view.dayChange);
        change.textContent = pct(view.dayChange);
      }
    });
  }

  function render() {
    const gem = gemForUnit(currentGem());
  ...
  function drawChart(progress = 1, hoverIndex = null) {
    ...
    for (let index = 0; index < 5; index += 1) {
      ...
      context.fillText(priceShort(max - (index / 4) * span), pad.left - 9, yy);
    }
    ...
    context.stroke();
    context.setLineDash([]);
    const currentLabel = priceShort(points.at(-1).value);
