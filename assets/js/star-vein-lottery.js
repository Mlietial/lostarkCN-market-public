...
function buildStarVeinPackCollage(){
  ...
  const cards=packs.map(pack=>{
    ...
    const images=[pack.cover,pack.detail].filter(Boolean).map((path,index)=>`<img class="${index?"share-collage-detail":"share-collage-cover"}" src="${base}${escapeShareText(path)}" alt="${escapeShareText(pack.name)}${index?"详情":"封面"}" loading="eager" decoding="sync">`).join("");
...
function renderExpectedPricing(){const averageExpectedCoins=(customAverageExpectedCoins??calculatedExpectedCoins/selectedMultiplier)*selectedMultiplier;const defaultAverageCostRmb=10*selectedMultiplier;const totalExpectedCoins=customExpectedCoins??calculatedExpectedCoins;const totalCostRmb=customExpectedCostRmb??defaultAverageCostRmb;const expectedCoins=expectedValueMode==="average"?averageExpectedCoins:totalExpectedCoins;const costRmb=expectedValueMode==="average"?defaultAverageCostRmb:totalCostRmb;expectedCoinsPerRmb=expectedCoins/costRmb;localStorage.setItem("starVeinExpectedCoinsPerRmb",String(expectedCoinsPerRmb));document.querySelector("#exploreSummary").innerHTML=expectedValueMode==="average"?`<strong>${selectedMultiplier} 倍平均值 · 推进 ${selectedRooms} 房</strong><span>${customAverageExpectedCoins===null?"理论平均获得":"填写平均获得"} <b>${money(expectedCoins)} 星脉币</b></span><span>${selectedMultiplier} 倍花费 <b>${money(costRmb)} 元</b></span><span>换算比例 <b>${expectedCoinsPerRmb.toFixed(2)} 星脉币 / 元</b></span>`:`<strong>总值计算 · 推进 ${selectedRooms} 房</strong><span>${customExpectedCoins===null?"当前倍率理论总值":"填写获取总值"} <b>${money(expectedCoins)} 星脉币</b></span><span>${customExpectedCostRmb===null?"当前倍率默认花费":"填写实际花费"} <b>${money(costRmb)} 元</b></span><span>换算比例 <b>${expectedCoinsPerRmb.toFixed(2)} 星脉币 / 元</b></span>`;renderPacks();}
function renderRooms(){let dist=new Map([[0,1]]);const stats=[];let html="";for(let r=0;r<5;r++){const next=new Map();for(const [coins,p0] of dist){const taxed=[[Math.floor(coins*.5),p0*taxRates[r]],[coins,p0*(1-taxRates[r])]];for(const [after,pTax] of taxed){if(!pTax)continue;roomRates[r].forEach((p,i)=>{const value=qualities[i][3]*selectedMultiplier;next.set(after+value,(next.get(after+value)||0)+pTax*p);});}}dist=next;const cumulative=[...dist].reduce((s,[v,p])=>s+v*p,0);stats.push(cumulative);const state=r+1===selectedRooms?" selected":r+1>selectedRooms?" beyond":"";html+=`<article class="room${state}"><h3>推进 ${r+1} 房</h3><div class="ev">${money(cumulative)} <small>星脉币</small></div><small>计入各房间奖励概率与税收事件后的累计获取均值</small></article>`;}document.querySelector("#roomGrid").innerHTML=html;calculatedExpectedCoins=stats[selectedRooms-1];document.querySelector("#averageExpectedLabel").textContent=`${selectedMultiplier} 倍平均获得`;document.querySelector("#averageExpectedUnit").textContent=`星脉币 / ${10*selectedMultiplier} 元`;document.querySelector("#averageExpectedCoins").value=String(Number(((customAverageExpectedCoins??calculatedExpectedCoins/selectedMultiplier)*selectedMultiplier).toFixed(2)));document.querySelector("#totalExpectedCoins").value=String(customExpectedCoins??Number(calculatedExpectedCoins.toFixed(2)));document.querySelector("#expectedCostRmb").value=String(customExpectedCostRmb??10*selectedMultiplier);renderExpectedPricing();}
...
function init(){
  ...
  document.querySelector("#averageExpectedCoins").addEventListener("input",event=>{
    ...
    customAverageExpectedCoins=Number.isFinite(value)&&value>0?value/selectedMultiplier:null;
    renderExpectedPricing();
  });
  document.querySelector("#averageExpectedCoins").addEventListener("blur",event=>{
    if(customAverageExpectedCoins===null)event.target.value=String(Number(calculatedExpectedCoins.toFixed(2)));
  });
  ...
  document.querySelector("#expectedCostRmb").addEventListener("input",event=>{
    const value=Number(event.target.value);
    customExpectedCostRmb=Number.isFinite(value)&&value>0?value:null;
    renderExpectedPricing();
  });
